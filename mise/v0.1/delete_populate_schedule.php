...
<?php
    ...
    $selectQuery = "SELECT name FROM actionschedule WHERE id = $id";
    ...
    if ($result && $result->num_rows > 0) {
        ...
        if ($first_name !== 'default') {
            ...
            $deleteQuery = "DELETE FROM actionschedule WHERE id = $id";
            if ($mysqli->query($deleteQuery)) {
                $response = "Schedule deleted successfully.";
            } else {
                $response = "Error deleting schedule: " . $mysqli->error;
            }
        } else {
